...
> **Superseded in part by [2026-06-03-plugins-design.md](2026-06-03-plugins-design.md):**
> the inline `mcps` and `skills` profile fields described below were removed in favor of
> plugins as the single capability-bundling primitive, the `market:`/`local:` reference
> syntax was replaced by `marketplace/plugin` shorthand and local paths, and the
> user-level/global scope was dropped — everything a profile applies lands inside the
> target repo's working tree (instructions go to a managed region of `CLAUDE.md` or
> `AGENTS.md`, not user-level files). The profile/overlay split, merge rules, state
> model, command surface, and ephemeral session lifecycle below shipped as designed.
