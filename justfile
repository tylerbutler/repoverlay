...
# Generate a CycloneDX SBOM (writes repoverlay.cdx.json)
sbom *ARGS='':
    cargo cyclonedx --format json --spec-version 1.5 --target all {{ARGS}}

# Verify GitHub artifact/SBOM attestations for a released file
sbom-attest-verify ARTIFACT:
    gh attestation verify {{ARTIFACT}} --repo tylerbutler/repoverlay

# Regenerate the autogenerated release.yml from dist-workspace.toml
dist-generate:
    dist generate

# Validate the dist release config locally
dist-plan:
    dist plan
